...
shift; at most 65536 expanded words/depth 16 may enter a kernel. Balanced-depth
...
the pure formula's ignored-tail behavior. Initial input vectors are reserved fallibly after metadata, budget and
shape checks. Further bounded kernel allocations remain ordinary allocations.
...
Eight new regressions cover hostile exponents 17/32/63/64/65/2^32/p-1, malformed
...
mismatch, compact depth 16 shared DAG expansion, low budgets, ignored point tails,
...
No GPU acceleration or complete 36-jet genesis implementation is implied by
these results. Work is on `fix/jet-input-admission`, without publishing or
committing unrelated owner manifest/lock changes.

The default-feature suite passed 169 tests with zero warnings. The optional acpu
...
Rust 1.98 (`/tmp/nox-linux-allfeatures-check.log`); native all-features compilation
