...
mod tests {
    ...
    fn make_xor<const N: usize>(ar: &mut Order<N>, a: u64, b: u64) -> crate::noun::NounId {
        let t = ar.atom(g(11), Tag::Field).unwrap();
    ...
    fn xor_basic() {
        ...
        match reduce(&mut ar, obj, formula, 1000, &NullCalls, &mut NoTrace) {
            Outcome::Ok(r, _) => assert_eq!(ar.atom_value(r).unwrap().0.as_u64(), 0b0110),
            o => panic!("{:?}", o),
        }
    }
