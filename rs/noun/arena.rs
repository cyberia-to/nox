...
use nebu::Goldilocks;
use super::tag::Tag;
use super::inner::NounInner;
use super::hash::{Digest, hash_atom, hash_cell};
use super::{NounRef, NIL};
...
impl<const N: usize> Arena<N> {
    ...
    pub fn head(&self, r: NounRef) -> Option<NounRef> {
        match self.get(r).inner { NounInner::Cell { left, .. } => Some(left), _ => None }
    }

    pub fn tail(&self, r: NounRef) -> Option<NounRef> {
        match self.get(r).inner { NounInner::Cell { right, .. } => Some(right), _ => None }
    }

    pub fn atom_value(&self, r: NounRef) -> Option<(Goldilocks, Tag)> {
        match self.get(r).inner { NounInner::Atom { value, tag } => Some((value, tag)), _ => None }
    }

    pub fn digest(&self, r: NounRef) -> &Digest { &self.get(r).hash }
}
